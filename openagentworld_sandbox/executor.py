...
class SafeExecutor:
    ...
    def __init__(
        ...
        max_history: int = 100,
        on_success: Callable[[ExecutionResult], None] = None,
        on_error: Callable[[ExecutionResult], None] = None,
    ...
    def _inject_dependencies(self, code: str) -> str:
        ...
        install_code = "\n".join([
            f"import subprocess; subprocess.run(['pip', 'install', '-q', '{pkg}'], check=True)"
    ...
    def run(self, code: str) -> ExecutionResult:
        ...
        # 6. Callbacks
        if result.exit_code == 0 and self.on_success:
            self.on_success(result)
        elif result.exit_code != 0 and self.on_error:
    ...
    async def run_streaming(self, code: str, callback: Callable[[str], None]) -> ExecutionResult:
        """
